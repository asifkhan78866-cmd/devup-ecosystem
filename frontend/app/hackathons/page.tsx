...
const HackathonArena = dynamic(() => import("@/components/3d/HackathonArena"), {
  ssr: false,
});
...
export default function HackathonsPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/hackathons`,
      ),
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/hackathons/featured`,
      ),
    ])
      .then(async ([listRes, featuredRes]) => {
        ...
        if (listData.success && listData.data) {
          const formatted = listData.data.map((h: any, i: number) => ({
            ...
            daysLeft: h.registrationDeadline
              ? getDaysLeft(h.registrationDeadline)
              : h.endDate
                ? getDaysLeft(h.endDate)
                : 30,
        ...
        if (featuredData.success && featuredData.data) {
          ...
          const end = new Date(
            featuredData.data.registrationDeadline,
          ).getTime();
          const now = Date.now();
  ...
  const filteredHackathons = hackathons.filter((h) => {
    ...
    if (activeFilter === "Ecosystem-hosted")
      return h.isEcosystemHosted === true;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      {/* Announcement Modal (shows once per session when DevThon is present) */}
      {showAnnouncement && (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-3 text-sm text-[#d0d0d0] leading-relaxed">
                  We understand that many of you have requested a change in the
                  schedule due to exams and other academic commitments. We
                ...
                <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#0d0d0d] border border-white/6 rounded-xl p-4 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-[#e4e4e4]">
                        Details will be announced shortly — stay tuned and join
                        our communities to receive updates.
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {loading ? (
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 mb-4 relative z-10">
          <div className="bg-[#111111] border border-white/10 rounded-[20px] overflow-hidden flex flex-col md:flex-row relative shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
              ...
              <div className="w-full md:w-[320px] lg:w-[380px] h-[250px] md:h-auto relative overflow-hidden shrink-0 group">
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow: "0 2px 10px rgba(200,241,53,0.3)",
                  }}
                >
                  FEATURED
                </div>
            ...
            <div
              className={`p-6 sm:p-10 flex-1 border-b md:border-b-0 md:border-r border-white/5 relative z-10 bg-[#111111] ${featuredHackathon.bannerUrl ? "md:pl-8" : ""}`}
            >
              {!featuredHackathon.bannerUrl && (
                <div
              ...
              <div className="mb-6 relative">
                ...
                >
                  PRIZE POOL{" "}
                  {featuredHackathon.registrationFee &&
                    `· REGISTRATION FEE: ${featuredHackathon.registrationFee.toUpperCase()}`}
                </div>
              </div>

              <div className="flex flex-wrap gap-2 mb-8">
                <span
                  className="px-3 py-1.5 bg-white/5 backdrop-blur-md border border-white/10 text-[#e4e4e4] rounded-[8px] shadow-sm"
                  style={{
                    fontFamily: "var(--font-inter), sans-serif",
                    fontSize: "12px",
                    fontWeight: 500,
              ...
              <div className="flex items-center gap-3 mb-8 bg-white/5 border border-white/10 rounded-[10px] p-4 inline-flex">
                ...
                >
                  <span className="text-[#888]">Starts:</span>{" "}
                  {new Date(featuredHackathon.startDate).toLocaleDateString(
                    "en-IN",
                    { month: "short", day: "numeric" },
                  )}{" "}
                  <span className="mx-2 text-white/20">|</span>{" "}
                  <span className="text-[#888]">Ends:</span>{" "}
                  {new Date(featuredHackathon.endDate).toLocaleDateString(
                    "en-IN",
                    { month: "short", day: "numeric" },
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-4 mb-8">
                ...
                <Link href={`/hackathons/${featuredHackathon.id}`}>
                  ...
                  >
                    Learn More{" "}
                    <span className="inline-block transition-transform group-hover:translate-x-1 ml-1 text-[#c8f135]">
                      →
                    </span>
                  </button>
                </Link>
              ...
              {/* Presented By Strip */}
              {featuredHackathon.partners &&
                featuredHackathon.partners.length > 0 && (
            ...
            <div className="p-6 sm:p-10 md:w-[380px] bg-white/[0.02] backdrop-blur-xl flex flex-col items-center justify-center relative overflow-hidden z-10 border-t md:border-t-0 md:border-l border-white/10">
              ...
              {new Date(featuredHackathon.registrationDeadline).getTime() <
              Date.now() ? (
                    ...
                    {[
                      ...
                      >
                        <div className="flex flex-col items-center">
                          <div className="w-[clamp(44px,14vw,72px)] h-[clamp(44px,14vw,72px)] bg-white/5 border border-white/10 rounded-[12px] flex items-center justify-center overflow-hidden relative shadow-lg backdrop-blur-sm">
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                        ...
                        >
                          {isClosed
                            ? "Registration closed"
                            : `${hackathon.daysLeft}d left to register`}
                        </div>
