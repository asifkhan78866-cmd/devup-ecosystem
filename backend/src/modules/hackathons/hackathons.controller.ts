...
export class HackathonsController {
  ...
  async updateHackathon(req: Request, res: Response) {
    const data = await hackathonsService.updateHackathon(req.params.id, req.body);
    res.status(200).json({ success: true, data });
  }
  ...
  async uploadBanner(req: Request, res: Response) {
    if (!req.file) throw new AppError(400, "No file uploaded");
    const data = await hackathonsService.uploadImage(req.params.id, "banner", req.file.buffer, req.file.mimetype);
    res.status(200).json({ success: true, data });
  }

  async register(req: Request, res: Response) {
    const data = await hackathonsService.register(req.params.id, req.user!.id, req.body);
    res.status(201).json({ success: true, data });
  }
}
